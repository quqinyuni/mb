...
// Exposes the constructors globally.
    ...
    u2u: function (url) {
        ...
        if (this.config["回传"]) {
            ...
            url = this.config["设备传参"] ? (this.config["MAC"] ? url.replace(/(__MAC1__|\{mac1\})/gi, this.Gvar.mdMac1).replace(/(__MAC__|\{mac\})/gi, this.config["设备加密"] ? this.Gvar.mdMac : this.Gvar.mac) : url) : url;
