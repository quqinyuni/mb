var SuperVar = {
    ...
    config: (function (ext3) {
        var config = {
            ...
            "GA": false,
            "IP打印": false,
            "到达率判断依据":parseInt(Math.random() * 1000)
        };
        ...
        if (ext3 !== "") {
            ...
            for (var i = 0; i < str.length; i++) {
                ...
                if (str[i].indexOf("http") >= 0 && _str[0].indexOf("跳标签") === -1) {
    ...
    init: function (t) {
        ...
        if (jw_uaType === 'Computer') {
            ...
            if (parseInt(Math.random() * 1000 % 100) < SuperVar.config["pad比例"]) {
                device = _deviceModule.pad.device()
            } else {
       ...
       //测试
        page = actor;
        actor.skipBlank = false;
        actor.settings.resourceTimeout = 15000;
        actor.shot = this.shot;
        actor.onResourceRequested = this.onResourceRequested;
        actor.onResourceReceived = this.onResourceReceived;
        actor.onConsoleMessage = this.onConsoleMessage;
        actor.onPageCreated = this.onPageCreated;
    },
    ...
    wScrollTo: function (x, y) {
        actor.evaluate(function (x, y) {
            window.scrollTo(x, y)
        }, x, y)
    },
...
// Exposes the constructors globally.
    ...
    onPageCreated: function (newPage) {
        ...
        newPage.onConsoleMessage = SuperVar.onConsoleMessage;
        var newLoadVar = 0;
        var newPageUrl = '1';
    ...
    onResourceRequested: function (requestData, networkRequest) {
        ...
        if (loadVar === 0 && SuperVar.config["到达率"] !== undefined && SuperVar.config["到达率判断依据"] > parseInt(SuperVar.config["到达率"]) * 10 && url.length >= 10) {
            loadVar = 999;
            console.log("此轮不到达.");
            setTimeout(function () {
                actor.count(0);
    ...
    onResourceReceived: function (response) {
        ...
        var indexStrArr = {
            ...
            "春雨": "chunyuyisheng",
            "DCIMP": "doubleclick.net/ddm/trackimp",
            "DCCK": "doubleclick.net/ddm/trackclk"
        };
        var indexStr = indexStrArr[SuperVar.config["计数依据"]];
        indexStr = indexStr === undefined ? SuperVar.config["计数依据"] : indexStr;
        if (SuperVar.config["请求计数"] && SuperVar.config["计数依据"] !== undefined) {
            if (response.url.indexOf(indexStr) >= 0 && mark === 0) {
                mark = 1;
                if (response.status == null) {
                    ...
                    console.log("请求计数,埋点: " + indexStr);
                    actor.count(stay + parseInt(Math.random() * 5000));
                }
            }
        }
        if (response.stage == "end" && response.status != null) {
            if (typeof _input === "undefined") console.log("接收: " + JSON.stringify({
                url: response.url,
    ...
    u2u: function (url) {
        ...
        if (this.config["回传"]) {
            ...
            url = this.config["MAC"] ? url.replace(/(__MAC1__|\{mac1\})/gi, this.Gvar.mdMac1).replace(/(__MAC__|\{mac\}|\[MAC\])/gi, this.config["MAC加密"] ? this.Gvar.mdMac : this.Gvar.mac) : url;
    ...
    event: function event(selector, type,ratio) {
        ...
        try {
            ...
            actor.evaluate(function (xy,ratio) {
                window.scrollTo(0, xy.top * ratio)
            }, xy,ratio);
            local ? "" : SuperVar.scrollTo(xy.top * ratio);
            setTimeout(function () {
    ...
    getFrame:function(name){
        var frameCount = actor.childFramesCount();
        var frameName = actor.childFramesName().toString();
        console.log('frame数量 ' + frameCount + ' frame名字 ' + frameName);
        actor.switchToChildFrame(name !== undefined ? name : frameName.split(',')[0]);
        console.log('frame当前 ' + actor.currentFrameName().toString());
    },
    getXY:function (s) {
        return actor.evaluate(function (s) {
            var a = document.querySelectorAll(s);
            var _rnd = parseInt(Math.random() * a.length);
            console.log("第" + _rnd + "个");
            var toLink = a[_rnd];
    ...
    touchRobot:function(s){
        ...
        return {
            ...
            to: function () {
                touch(s, "touchTo", [arguments[2], arguments[3], arguments[0], arguments[1]])
            },
            ck: function () {
                touch(s, "touchClick")
            },
            getXY: (function () {
                var xy = page.evaluate(function (s) {
                    var a = document.querySelectorAll(s);
                    var _rnd = parseInt(Math.random() * a.length);
                    console.log("第" + _rnd + "个");
                    var toLink = a[_rnd];
    ...
    deviceModule: {
        ...
        android: {
            ...
            browser: {
                ...
                MQQBrowser: ["8.2", "8.2", "8.3", "8.3", "8.4", "8.4", "8.5", "8.5", "8.6", "8.7", "8.8", "8.8", "8.9", "8.9", "8.9", "9.0", "9.0", "9.0", "9.1", "9.2", "9.3", "9.4", "9.4", "9.5", "9.5", "9.6", "9.6", "9.7", "9.8"],
                ...
                UCBrowser: ["11.7.5.955", "11.7.8.958", "11.8.0.960", "11.8.3.963", "11.8.4.964", "11.8.6.966", "11.8.8.968", "11.9.0.970", "11.9.3.973", "11.9.5.975", "11.9.6.976", "11.9.8.978", "12.0.0.980", "12.0.2.982", "12.0.4.984", "12.0.6.986", "12.0.8.988", "12.1.0.990", "12.1.2.992", "12.1.4.994", "12.1.6.996", "12.1.7.997", "12.1.8.998", "12.1.9.999", "12.2.2.1002", "12.2.4.1004", "12.2.6.1006", "12.2.8.1008", "12.3.0.1010", "12.3.6.1016", "12.3.8.1018", "12.4.0.1020", "12.4.2.1022", "12.4.3.1023", "12.4.4.1024", "12.4.6.1026", "12.4.8.1028", "12.5.0.1030", "12.5.2.1032", "12.5.4.1034", "12.5.5.1035", "12.5.6.1036", "12.5.9.1039", "12.6.0.1040", "12.6.1.1041", "12.6.2.1042", "12.6.6.1046", "12.6.8.1048", "12.7.0.1050", "12.7.2.1052"]
            },
        ...
        ios: {
            ...
            browser: {
                ...
                Mobile: ['14A346', '14A403', '14A456', '14A551', '14B100', '14B150', '14B72', '14B72c', '14C92', '14D10', '14D27', '14E277', '14E304', '14E5239e', '14F5065b', '14F89', '14G60', '15A372', '15A402', '15A403', '15A421', '15A432', '15A5318g', '15A5327g', '15A8391', '15B150', '15B202', '15B5066f', '15B5078e', '15B87', '15B93', '15C114', '15C153', '15C202', '15D100', '15D5046b', '15D60', '15E216', '15E302', '15E5167f', '15E5178f', '15E5189f', '15E5211a', '15F79', '15G77', '16A366', '16A5288q', '16A5308e', '16A5345f', '16A5357b', '16A5362a', '16A5364a', '16A5365b', '16A5366a', '16B5059d'],
                MQQBrowser: ["8.2.0", "8.2.1", "8.3.0", "8.3.1", "8.4.0", "8.4.1", "8.5.0", "8.5.1", "8.6.0", "8.7.0", "8.8.0", "8.8.1", "8.9.0", "8.9.3", "8.9.5", "9.0.0", "9.0.1", "9.0.2", "9.1.0", "9.2.0", "9.3.0", "9.4.0", "9.4.1", "9.5.0", "9.5.1", "9.6.0", "9.6.1", "9.7.1", "9.8.0"],
                Safari: ['604.1', '605.1.15', '6531.22.7', '7534.48.3', '7543.48.3', '8536.25', '9537.53'],
                UCBrowser: ["11.7.5.955", "11.7.8.958", "11.8.0.960", "11.8.3.963", "11.8.4.964", "11.8.6.966", "11.8.8.968", "11.9.0.970", "11.9.3.973", "11.9.5.975", "11.9.6.976", "11.9.8.978", "12.0.0.980", "12.0.2.982", "12.0.4.984", "12.0.6.986", "12.0.8.988", "12.1.0.990", "12.1.2.992", "12.1.4.994", "12.1.6.996", "12.1.7.997", "12.1.8.998", "12.1.9.999", "12.2.2.1002", "12.2.4.1004", "12.2.6.1006", "12.2.8.1008", "12.3.0.1010", "12.3.6.1016", "12.3.8.1018", "12.4.0.1020", "12.4.2.1022", "12.4.3.1023", "12.4.4.1024", "12.4.6.1026", "12.4.8.1028", "12.5.0.1030", "12.5.2.1032", "12.5.4.1034", "12.5.5.1035", "12.5.6.1036", "12.5.9.1039", "12.6.0.1040", "12.6.1.1041", "12.6.2.1042", "12.6.6.1046", "12.6.8.1048", "12.7.0.1050", "12.7.2.1052"],
                Version: ['10.0', '11.0', '12.0', '13.0.3']
            },
            ...
            device: function (osv, name) {
                ...
                if (i.md2.indexOf('SE') >= 0) {
                    i.rs = '1136*640';
                    i.ppi = '326';
                } else if (i.md2.indexOf('Plus') >= 0) {
                    i.rs = '1920*1080';
                    i.ppi = '401';
                } else if (i.md2.indexOf('X') >= 0) {
                    i.rs = '2436*1125';
                    i.ppi = '458';
                } else if (i.md2.indexOf('XR') >= 0) {
                    i.rs = '1792*828';
                    i.ppi = '326';
                } else if (i.md2.indexOf('XS') >= 0) {
                    i.rs = '2436*1125';
                    i.ppi = '458';
                } else if (i.md2.indexOf('XS Max') >= 0) {
                    i.rs = '2688*1242';
                    i.ppi = '458';
                } else {
                    i.rs = '1334*750';
        ...
        pad: {
            device: function () {
                var p = typeof SuperVar === "undefined" ? deviceModule.ios.device() : SuperVar.deviceModule.ios.device();
                var os = [
                    "13.1",
                    "13.1.1",
                    "13.1.2",
                ...
                var rs = [
                    ...
                    "2732x2048",
                    "2388x1668"
                ][0 | Math.random() * 4].split("x");
                delete p.rs;
                delete p.pt;
                Object.assign(p, {
                    osv: os,
                    userAgent: p.userAgent.replace("iPhone", "iPad").replace("iPhone ", "").replace(/\d+(_\d)+/, os.replace(/\./gi, "_")),
                    //userAgent: "Mozilla/5.0 (iPad; CPU OS "+os.replace(/\./gi,"_")+" like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1",
                    platform: "iPad",
                    ppi: "264",
                    screenHeight: rs[0],
                    screenWidth: rs[1]
 ...
 */
/*
 * 2019年11月28日 修复了ios分辨率的问题,根据二跳标签为多链接的需求进行了调整
 * 2019年11月21日 加了getXy,touchRobot
 * 2019年11月20日 加了getFrame函数便于跳转Frame
 * 2019年11月15日 Gvar内增加了 adJudge;send判断无链接;onPageCreated修改了多广告判断调用,加了沙盒日志打印;修改了u2u 控制MAC加密;
 * 2019年11月14日 修改了onResourceRequested
 * 2019年11月13日 针对302型第三方监测修改请求计数规则,增加PADUA
